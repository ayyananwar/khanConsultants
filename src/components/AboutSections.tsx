  ...
  return (
    <section id="story" className="py-12 sm:py-16 md:py-20 lg:py-24 xl:py-28 px-3 sm:px-4 md:px-6" style={{ backgroundColor: designTokens.colors.neutral.white }}>
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8 sm:mb-10 md:mb-12 lg:mb-16 pb-6 sm:pb-8 md:pb-10 lg:pb-12" style={{ borderBottom: '2px solid var(--color-d6fadc)' }}>
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold" style={{ color: 'var(--color-111827)' }}>
            Our Journey
          </h2>
        </div>
          ...
          {milestones.map((milestone, index) => (
            <div key={index} className="relative flex gap-4 sm:gap-5 md:gap-6 lg:gap-8">
              <div className="flex flex-col items-center">
                <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-full flex items-center justify-center font-bold text-white flex-shrink-0 text-sm sm:text-base md:text-lg" style={{ background: 'var(--color-111827)' }}>
                  {milestone.year % 10}
                </div>
                {index < milestones.length - 1 && (
                  <div className="w-1 h-12 sm:h-14 md:h-16 lg:h-20" style={{ backgroundColor: 'var(--color-d6fadc)' }}></div>
                )}
              </div>

              <div className="flex-1 pb-4 sm:pb-5 md:pb-6 lg:pb-8">
                <div className="rounded-xl sm:rounded-2xl p-4 sm:p-5 md:p-6 lg:p-8 transition-all border-2 hover:shadow-xl hover:-translate-y-2" style={{ background: 'linear-gradient(to bottom right, white, rgba(214, 250, 220, 0.2))', borderColor: 'var(--color-e8f0eb)' }}>
                  <p className="font-bold text-lg sm:text-xl md:text-2xl" style={{ color: 'var(--color-111827)' }}>{milestone.year}</p>
                  <h3 className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold mt-1 sm:mt-2 mb-1 sm:mb-2" style={{ color: 'var(--color-111827)' }}>{milestone.title}</h3>
                  <p className="text-xs sm:text-sm md:text-base lg:text-lg" style={{ color: 'var(--color-4b5563)' }}>{milestone.description}</p>
                </div>
              </div>
            </div>
          ))}
...
function LeadershipTeam() {
  ...
  return (
    <section id="team" className="py-12 sm:py-16 md:py-20 lg:py-24 xl:py-28 px-3 sm:px-4 md:px-6" style={{ background: 'var(--color-3d6b56)' }}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 md:gap-6 lg:gap-8">
          {leaders.map((leader, index) => (
            ...
            >
              <div className="h-32 sm:h-36 md:h-40 flex items-center justify-center" style={{ background: 'linear-gradient(to bottom right, var(--color-2c4d3f), var(--color-3d6b56))' }}>
              ...
              <div className="p-4 sm:p-5 md:p-6 lg:p-7">
                ...
                <div className="flex gap-3 sm:gap-4 justify-center pt-3 sm:pt-4 md:pt-5" style={{ borderTop: '2px solid rgba(255,255,255,0.2)' }}>
                  <a
                    ...
                    style={{ color: 'var(--color-9ca3af)' }}
                    onMouseEnter={(e) => e.currentTarget.style.color = 'var(--color-0a66c2)'}
                    onMouseLeave={(e) => e.currentTarget.style.color = 'var(--color-9ca3af)'}
                  >
                  ...
                  <a
                    ...
                    style={{ color: 'var(--color-9ca3af)' }}
                    onMouseEnter={(e) => e.currentTarget.style.color = 'var(--color-ffffff)'}
                    onMouseLeave={(e) => e.currentTarget.style.color = 'var(--color-9ca3af)'}
                  >
...
function ValuesSection() {
  ...
  return (
    <section id="values" className="py-12 sm:py-16 md:py-20 lg:py-24 xl:py-28 px-3 sm:px-4 md:px-6" style={{ backgroundColor: designTokens.colors.neutral.white }}>
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-8 sm:mb-10 md:mb-12 lg:mb-16 pb-6 sm:pb-8 md:pb-10 lg:pb-12" style={{ borderBottom: '2px solid var(--color-d6fadc)' }}>
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-2 sm:mb-3" style={{ color: 'var(--color-111827)' }}>
            Our Core Values
          </h2>
          <p className="text-xs sm:text-sm md:text-base lg:text-lg max-w-2xl mx-auto" style={{ color: 'var(--color-4b5563)' }}>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 md:gap-6 lg:gap-8">
          {values.map((value, index) => (
            <div
              ...
              style={{
                background: 'linear-gradient(to bottom right, white, rgba(214, 250, 220, 0.2))',
                borderColor: 'var(--color-e8f0eb)',
              }}
            >
              <div className="flex justify-center mb-4 sm:mb-5 md:mb-6 text-3xl sm:text-4xl md:text-5xl">{value.icon}</div>
              <h3 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold mb-2 sm:mb-3 text-center" style={{ color: 'var(--color-111827)' }}>{value.title}</h3>
              <p className="leading-relaxed text-xs sm:text-sm md:text-base lg:text-lg text-center" style={{ color: 'var(--color-4b5563)' }}>{value.description}</p>
            </div>
          ))}
        </div>
      </div>
