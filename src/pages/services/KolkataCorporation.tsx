    ...
    <div className="bg-white">
      ...
      <section className="py-6 sm:py-8 md:py-12 lg:py-16 px-3 sm:px-4 md:px-6" style={{ backgroundColor: designTokens.colors.neutral.white }}>
        <div className="max-w-4xl mx-auto">
          <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-center mb-4 sm:mb-5 md:mb-6" style={{ color: designTokens.colors.sage[900] }}>Your Bridge to the Kolkata Municipal Corporation</h2>
          ...
          <div className="text-center mb-8 sm:mb-10 md:mb-12 lg:mb-16">
            <div className="inline-block">
              <p className="text-[10px] sm:text-xs md:text-sm tracking-[0.2em] text-[#53785b] font-bold mb-2 sm:mb-3 uppercase relative">
                <span className="relative z-10 bg-[#f0f4f1] px-3 sm:px-4">SERVICES UNDER KMC</span>
                <span className="absolute left-0 right-0 top-1/2 h-px bg-gradient-to-r from-transparent via-[#53785b]/30 to-transparent"></span>
              </p>
            </div>
            <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold mb-2 sm:mb-3" style={{ color: designTokens.colors.sage[900] }}>Our Core Services</h2>
            ...
            {service.coreServices.map((item, index) => {
              ...
              return (
                <div key={index} className="rounded-xl sm:rounded-2xl p-5 sm:p-6 md:p-7 lg:p-8 hover:shadow-lg transition bg-white border border-[#e8f0eb]">
                  ...
                  <h3 className="text-lg sm:text-xl font-bold mb-2 sm:mb-3" style={{ color: designTokens.colors.sage[900] }}>{item.title}</h3>
                  <p className="text-xs sm:text-sm mb-3 sm:mb-4" style={{ color: designTokens.colors.sage[600] }}>{item.description}</p>
                  <ul className="space-y-2 mb-5 sm:mb-6">
                    {item.features.map((feature, idx) => (
                      <li key={idx} className="flex items-start gap-2 text-xs sm:text-sm" style={{ color: designTokens.colors.sage[700] }}>
                        <span className="flex-shrink-0 font-bold" style={{ color: designTokens.colors.sage[600] }}>•</span>
                        <span>{feature}</span>
                      </li>
                    ))}
                  </ul>
                  <Link to={item.buttonLink} className="inline-block px-4 sm:px-5 py-2 sm:py-2.5 text-white rounded-lg font-semibold transition text-xs sm:text-sm" style={{ backgroundColor: designTokens.colors.sage[600] }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = designTokens.colors.sage[700]} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = designTokens.colors.sage[600]}>
                    {item.buttonText}
                  </Link>
                </div>
              );
          ...
          <div className="text-center mb-8 sm:mb-10 md:mb-12 lg:mb-16">
            <div className="inline-block">
              <p className="text-[10px] sm:text-xs md:text-sm tracking-[0.2em] text-red-700 font-bold mb-2 sm:mb-3 uppercase relative">
                <span className="relative z-10 bg-transparent px-3 sm:px-4">YOUR USP</span>
                <span className="absolute left-0 right-0 top-1/2 h-px bg-gradient-to-r from-transparent via-red-400/50 to-transparent"></span>
              </p>
            </div>
            <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-bold mb-2 sm:mb-3 md:mb-4 text-red-900 leading-tight">More Than Just Filing—We Handle the Legal Hurdles</h2>
            ...
            <p className="text-xs sm:text-sm md:text-base text-red-800 max-w-3xl mx-auto px-2">KMC processes often get stuck due to missing "legal links." Unlike standard agents, Khan Consultants is backed by a robust team of lawyers. We handle the drafting and execution of all necessary legal supports in-house:</p>
          </div>
      ...
      <section className="py-12 sm:py-16 md:py-20 lg:py-24 xl:py-28 px-3 sm:px-4 md:px-6 bg-[#d6fadc]">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="text-center mb-8 sm:mb-10 md:mb-12 lg:mb-16">
            ...
            <div className="w-16 sm:w-20 md:w-24 h-1 bg-gradient-to-r from-[#53785b] to-[#2c4d3f] mx-auto mb-4 sm:mb-6 md:mb-8 rounded-full"></div>
            <p className="text-sm sm:text-base md:text-lg text-[#53785b] px-2">Our approach is simple, transparent, and structured:</p>
          </div>
